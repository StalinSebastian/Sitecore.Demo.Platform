...
module.exports = {
    ...
    scriban: {
        path: (function () {
			...
			if(_path) {
				console.log("Scriban path:" + `${_path}${SCRIBAN_PATH}/**/*.scriban`);
        ...
        metadataFilePath: (function () {
              const _path = getPosixRelativePath('scriban');
              if(_path) {
                console.log("Scriban metadata path:" + `${_path}${SCRIBAN_PATH}/metadata.json`);
    ...
    stylesConfig: {
        ...
        'services': 'component-services.scss'
    },
    ...
    init: function () {
        ...
        const { configUtils } = require('@sitecore/sxa-celt');
...
function isCeltInstalled() {
    return fs.existsSync(path.resolve(__dirname, '../node_modules/@sitecore/sxa-celt'));
}

function postInit() {
    global.rootPath = path.resolve(__dirname, '../');

    require('@sitecore/sxa-celt/util/postInit');
}
